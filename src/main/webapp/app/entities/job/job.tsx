  ...
  return (
    <div>
      <h2 id="job-heading" data-cy="JobHeading">
        <Translate contentKey="jhipsterQuotesApp.job.home.title">Jobs</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" variant="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterQuotesApp.job.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/job/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterQuotesApp.job.home.createLabel">Create new Job</Translate>
          </Link>
        </div>
      </h2>
      <div className="table-responsive">
        {jobList?.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterQuotesApp.job.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="jhipsterQuotesApp.job.employee">Employee</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {jobList.map(job => (
                <tr key={`entity-${job.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/job/${job.id}`} variant="link" size="sm">
                      {job.id}
                    </Button>
                  </td>
                  <td>{job.jobTitle}</td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/job/${job.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterQuotesApp.job.home.notFound">No Jobs found</Translate>
            </div>
          )
        )}
      </div>
