...
import com.example.jhipsterquotes.domain.Job;
import com.example.jhipsterquotes.repository.JobRepository;
...
/**
 * REST controller for managing {@link com.example.jhipsterquotes.domain.Job}.
 */
@RestController
...
public class JobResource {
    ...
    public ResponseEntity<Job> partialUpdateJob(@PathVariable(value = "id", required = false) final Long id, @RequestBody Job job)
        ...
        Optional<Job> result = jobRepository
            .findById(job.getId())
            .map(existingJob -> {
                updateIfPresent(existingJob::setJobTitle, job.getJobTitle());
                updateIfPresent(existingJob::setMinSalary, job.getMinSalary());
                updateIfPresent(existingJob::setMaxSalary, job.getMaxSalary());
