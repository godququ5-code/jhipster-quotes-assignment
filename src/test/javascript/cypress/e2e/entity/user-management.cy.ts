...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Roslyn31', email: 'Maddison.Bergnaum12@yahoo.com' };
  ...
  describe.skip('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Toby43');
      cy.get(`[data-cy="login"]`).should('have.value', 'Toby43');

      cy.get(`[data-cy="firstName"]`).type('Lonzo');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Lonzo');

      cy.get(`[data-cy="lastName"]`).type('Herzog');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Herzog');

      cy.get(`[data-cy="email"]`).type('Sister_Rogahn44@yahoo.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Sister_Rogahn44@yahoo.com');
